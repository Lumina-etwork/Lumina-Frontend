...
export function ThroughputChart({
  ...
  const slidingWindowRef = useRef(new SlidingWindow<ChartDataPoint>(200));
  ...
  const {
    data: throttledMessages,
    push,
    metrics,
  } = useDataThrottle<PacketMessage>({
    intervalMs: 500,
    maxBufferSize: 1000,
  ...
  const useMock =
    typeof window !== "undefined" && wsUrl.includes("localhost:8080");
    ...
    if (enablePerformanceTracking) {
      ...
      if (now - lastLogTimeRef.current > 10000) {
        console.log("[ThroughputChart] Performance metrics:", {
          ...
          messagesPerRender: (
            messageCountRef.current / renderCountRef.current
          ).toFixed(2),
  ...
  }, [renderCountRef.current]); // Re-compute when window updates
      ...
      {chartData.length > 0 && (
        <div className="mt-4 grid grid-cols-3 gap-4 border-t border-[#ece5d8] pt-4">
          ...
          <div>
            ...
            <p className="mt-1 text-lg font-semibold text-[#1a1410]">
              {(
                chartData.reduce((sum, d) => sum + d.throughput, 0) /
                chartData.length
              ).toFixed(2)}
            </p>
            <p className="text-xs text-[#6f5f48]">packets/s</p>
